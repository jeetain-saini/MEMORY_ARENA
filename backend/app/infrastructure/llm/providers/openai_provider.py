"""OpenAIProvider — LLMProvider backed by the OpenAI Chat Completions API.

The SDK is imported lazily so the dependency is only required when this provider
is actually selected (the offline default is deterministic). Structured output
is requested as JSON and parsed; the ``schema`` keys are described in the prompt.
"""
