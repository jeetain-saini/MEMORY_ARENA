...
## 18. Stage 10 (Phase 4) — Query-Time Agent Runtime

Stage 10 Phase 4 adds the **query-time agent**: an orchestration layer that turns
a user query into an answer by driving the *existing* MemoryArena pipeline. It is
**not** a generic chatbot and adds **no new capability** — every stage delegates
to a service built in Stages 7–10 P3. **MemoryArena remains the system; the agent
only orchestrates it**, and the `ContextPackage` remains the primary artifact the
...
```
   User Query
       │
       ▼
   Agent Runtime  (sequential offline default / LangGraph)
       │   retrieve ─▶ expand ─▶ build context ─▶ generate ─▶ citations
       ▼
   Retrieval → Graph Expansion → Context Assembly → LLM Compression → Answer
```
...
backend/app/
├── application/
│   ├── dto/agent_dto.py                       # AgentRequest/Response/Message/ToolCall/State/StepResult/Trace/Config + Citation + StreamEvent
│   ├── interfaces/
│   │   ├── agent_runtime.py                    # AgentRuntime port: respond() + stream()
│   │   └── agent_tool.py                        # AgentTool ABC
│   ├── services/agent/
│   │   ├── tools.py                            # MemorySearchTool, GraphExpansionTool, ContextBuilderTool
│   │   ├── toolset.py                          # AgentToolSet
│   │   └── citation_validation.py             # dedup + id-validate + cap + provenance
│   └── use_cases/query_memory_use_cases.py(+_impl)  # QueryMemoryUseCase
├── infrastructure/llm/graphs/
│   ├── agent_steps.py                         # shared stages + guardrails + streaming + serialization
│   ├── sequential_agent_runtime.py            # SequentialAgentRuntime (offline default)
│   └── agent_graph.py                          # LangGraphAgentRuntime (lazy import)
├── schemas/query.py                            # QueryRequest/Response + Citation schemas
└── api/v1/routes/query.py                      # POST /query, POST /query/stream (SSE)
```

### 18.2 Agent architecture

`AgentRuntime` is a port with `respond()` (run to completion) and `stream()`
(yield events). Both runtimes operate on a single mutable `AgentState` threaded
through shared stage functions in `agent_steps.py`, so the sequential and
LangGraph runtimes never diverge (same pattern as `extraction_steps`). No
LangGraph type crosses the port — both methods take/yield plain DTOs.

`SequentialAgentRuntime` (offline default, `AGENT_RUNTIME=sequential`) runs the
stages in one linear pass: **no loops, no autonomous planning**.
`LangGraphAgentRuntime` (`AGENT_RUNTIME=langgraph`) wires the same stage
functions as a `StateGraph`, future-compatible with tool loops; `langgraph` is
imported lazily and its suite `importorskip`s the package.

### 18.3 Tool system

Three tools, each a **thin adapter over one existing service** with zero
business logic, aggregated in `AgentToolSet`:

| Tool | Wraps | Produces |
| --- | --- | --- |
| `MemorySearchTool` | `MemoryRetrievalService.search` | base hybrid hits |
| `GraphExpansionTool` | `GraphAwareRetrievalService.expand` | graph neighbors |
| `ContextBuilderTool` | `ContextBuilderService.build` | **`ContextPackage`** |

**Retrieval runs exactly once.** `GraphAwareRetrievalService` gained an `expand`
method (its `search` now retrieves then delegates to it) so the graph tool reuses
the base hits rather than retrieving again; `ContextBuilderService.build` gained
an optional `retrieved=` parameter so the builder consumes the agent's combined
candidate set instead of running a third retrieval. No subsystem is bypassed and
none runs twice.

### 18.4 Generation & citation validation

The answer is generated by `LLMProvider.generate()` from a prompt whose primary
body is `ContextPackage.context_text`, augmented with a graph-related section —
so generation provably consumes both context assembly and graph expansion.
Citations are then validated: **deduplicated** by `memory_id` (highest score
wins), **validated** against the set actually retrieved/expanded (a citation can
never reference an unseen memory), **capped** to `max_citations`, with
**provenance preserved** (`hybrid` vs `graph`).

### 18.5 Guardrails (termination strategy)

A run is a single linear pass; guards bound it and set `finish_reason`:

- **max_iterations** — planning-round ceiling (default 1); guards future loops.
- **max_tool_calls** — total tool invocations across the run (default 8).
- **token** — context guaranteed ≤ `max_tokens` by the builder; the answer is
  truncated to `answer_max_tokens`.
- **timeout** — the whole run is wrapped in `asyncio.wait_for`.

`finish_reason ∈ {completed, timeout, max_iterations, max_tool_calls, error}`.
**Tool-failure recovery:** a failed retrieval or graph expansion is
**non-critical** — the run degrades (continues with whatever it has) and still
answers; a failed context build is **terminal** (`error`), since the
`ContextPackage` is the primary artifact. Tools never raise for service failures;
they return a non-ok step the runtime acts on.

### 18.6 Streaming architecture

`POST /query/stream` returns a `StreamingResponse` of **Server-Sent Events**. The
runtime's `stream()` yields a `step` event per stage, then `answer`, `citations`,
and always a terminal `done` (carrying `finish_reason`); an `error` event
precedes `done` on failure or timeout. The overall timeout wraps each step;
client disconnect surfaces as `CancelledError` and closes the underlying
generator cleanly. The route serializes each event to an `event:/data:` frame.

### 18.7 Query use case & API

`QueryMemoryUseCase` invokes the runtime and returns the `AgentResponse` — no
infrastructure access. `POST /query` returns `{answer, citations}` in the
standard envelope; `POST /query/stream` streams SSE. The runtime is assembled at
the composition root (`providers.py`) from the existing service providers; no
background processor or lifespan wiring is needed (query is synchronous
request/response).

### 18.8 Tests

Unit: DTOs; the three tools + tool set; citation validation (dedup, id-validate,
cap, provenance); the sequential runtime (full flow, ContextPackage-as-primary,
graph consumption, single-retrieval, every guard, and the three tool-failure
recovery paths); streaming (event sequence, error/timeout frames, always-`done`);
the query use case (delegation). LangGraph parity + guards are skip-guarded.
