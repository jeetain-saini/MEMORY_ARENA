...
const KIND_META: Record<MemoryEventKind, { label: string; icon: LucideIcon; color: string }> = {
  created: { label: "Created", icon: Plus, color: "text-emerald-300" },
  updated: { label: "Updated", icon: Pencil, color: "text-sky-300" },
  promoted: { label: "Promoted", icon: Star, color: "text-amber-300" },
  archived: { label: "Archived", icon: Archive, color: "text-zinc-300" },
  superseded: { label: "Superseded", icon: ArrowRightLeft, color: "text-violet-300" },
  forgotten: { label: "Forgotten", icon: EyeOff, color: "text-orange-300" },
  summary: { label: "Summary", icon: ScrollText, color: "text-indigo-300" },
};
                ...
                {group.events.map((e) => {
                  ...
                  return (
                    <li key={e.id} className="relative">
                      ...
                      <span className="absolute -left-[31px] top-3 flex h-5 w-5 items-center justify-center rounded-full border border-white/15 bg-background">
                        <Icon className={cn("h-3 w-3", color)} />
                      </span>
                      <div className="lift rounded-xl border border-white/10 bg-white/[0.035] p-4 backdrop-blur-xl">
          ...
          {visible < filtered.length ? (
            <div className="flex justify-center">
              <button
                ...
                className="rounded-full border border-white/15 bg-white/[0.05] px-5 py-2 text-sm font-medium text-foreground transition-colors hover:bg-white/[0.09]"
              >
